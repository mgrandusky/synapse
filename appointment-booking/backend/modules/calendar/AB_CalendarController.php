...
class AB_CalendarController extends AB_Controller  {
    ...
    public function index() {
        ...
        wp_localize_script( 'ab-jquery.weekcalendar.js', 'BookL10n', array(
        ...
        if (is_array($staff_ids)) {
            ...
            foreach ( $schedule as $item ) {
                ...
                $holidays = $wpdb->get_results($wpdb->prepare(
                        'SELECT * FROM ab_holiday WHERE staff_id = %d AND ((`repeat_event` = 0 and `date` = %s) OR (`repeat_event` = 1 AND DATE_FORMAT( `date` , "%%Y-%%m" ) = %s))',
